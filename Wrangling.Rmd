...
```{r}
arrange(Teams, yearID)
arrange(Teams, desc(yearID))
```
Teams has the year range of 1871 to 2019 

What date range does our "Salaries" dataset cover? (4 pts.)

```{r}
arrange(Salaries, yearID)
arrange(Salaries, desc(yearID))
```
Salaries has a year range of 1985 to 2016
...
```{r}
joined <- left_join(by_teams_year, teams, by = NULL)
...
> ANS: The expected number of wins
...
> ANS: The Oakland A's were far more efficient
